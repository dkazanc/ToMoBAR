...
for projector in ("astra", "fourier"):
    # %%
    print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
    print(f"Reconstructing with FISTA OS-TV (PD) method ({projector} projector)")
    print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
    ...
    Run_time = toc - tic
    print(
        "FISTA OS-TV reconstruction ({} projector) done in {} seconds".format(
            projector, Run_time
        )
    )
    ...
    plt.title(f"FISTA OS-TV (PD) Reconstruction ({projector} projector), axial view")

    plt.subplot(132)
    plt.imshow(RecFISTA_os_tv[:, sliceSel, :], vmin=0, vmax=max_val)
    plt.title(f"FISTA OS-TV (PD) Reconstruction ({projector} projector), coronal view")

    plt.subplot(133)
    plt.imshow(RecFISTA_os_tv[:, :, sliceSel], vmin=0, vmax=max_val)
    plt.title(f"FISTA OS-TV (PD) Reconstruction ({projector} projector), sagittal view")
    plt.show()
    ...
    RMSE = Qtools.rmse()
    print(
        "Root Mean Square Error is {} for FISTA OS-TV (PD) reconstruction ({} projector)".format(
            RMSE, projector
        )
    )
    # %%
    print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
    print(f"Reconstructing with ADMM OS-TV (PD) method ({projector} projector)")
    print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
    RectoolsCuPy = RecToolsIRCuPy(
        DetectorsDimH=Horiz_det,  # Horizontal detector dimension
        DetectorsDimH_pad=0,  # Padding size of horizontal detector
    ...
    Run_time = toc - tic
    print(
        "ADMM OS-TV reconstruction ({} projector) done in {} seconds".format(
            projector, Run_time
        )
    )
    ...
    plt.title(f"ADMM OS-TV (PD) Reconstruction ({projector} projector), axial view")

    plt.subplot(132)
    plt.imshow(RecADMM_os_tv[:, sliceSel, :], vmin=0, vmax=max_val)
    plt.title(f"ADMM OS-TV (PD) Reconstruction ({projector} projector), coronal view")

    plt.subplot(133)
    plt.imshow(RecADMM_os_tv[:, :, sliceSel], vmin=0, vmax=max_val)
    plt.title(f"ADMM OS-TV (PD) Reconstruction ({projector} projector), sagittal view")
    plt.show()
    ...
    RMSE = Qtools.rmse()
    print(
        "Root Mean Square Error is {} for ADMM OS-TV (PD) reconstruction ({} projector)".format(
            RMSE, projector
        )
    )
