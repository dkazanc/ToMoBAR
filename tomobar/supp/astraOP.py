...
def _setOS_indices(self):
    ...
    self.NumbProjBins = (int)(
        np.ceil(float(AnglesTot) / float(self.OS_number))
    )  # get the number of projections per bin (subset)
    self.newInd_Vec = np.zeros(
        [self.OS_number, self.NumbProjBins], dtype="int"
...
def _set_OS_geometry2d(self):
    ...
    for sub_ind in range(self.OS_number):
        ...
        if np.ndim(self.CenterRotOffset) == 0:
            ...
            vectorsOS = vec_geom_init2D(
                anglesOS, 1.0, self.CenterRotOffset[self.indVec]
            )
        self.proj_geom_OS[sub_ind] = astra.create_proj_geom(
            "parallel_vec", self.DetectorsDimH, vectorsOS
        )
...
def _set_OS_geometry3d(self):
    ...
    for sub_ind in range(self.OS_number):
        ...
        if np.ndim(self.CenterRotOffset) == 0:  # CenterRotOffset is a _scalar_
            ...
            vectors = vec_geom_init3D(
                anglesOS, 1.0, 1.0, self.CenterRotOffset[self.indVec]
            )
        self.proj_geom_OS[sub_ind] = astra.create_proj_geom(
            "parallel3d_vec", self.DetectorsDimV, self.DetectorsDimH, vectors
        )
...
class Astra3D:
    ...
    def runAstraBackprojCuPy(
        self, proj_data: xp.ndarray, method: str, os_index: int
    ) -> xp.ndarray:
    ...
    def runAstraProjCuPy(self, volume_data: xp.ndarray, os_index: int) -> xp.ndarray:
