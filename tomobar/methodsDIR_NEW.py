...
class RecToolsDIR():
    ...
    def __init__(
        ...
        device_projector, GPUdevice_index = parse_device_argument(
            device_projector
        )
    ...
    def FBP(self, data: np.ndarray) -> np.ndarray:
        ...
        if self.geom == "2D":
            ...
            if self.Atools.processing_arch == "gpu":
                return self.Atools._fbp(data)  # GPU reconstruction
            else:                
                return self.Atools._backproj(_filtersinc2D(data)) #backproject
        else:
            return self.Atools._backproj(_filtersinc3D(data))  # backproject